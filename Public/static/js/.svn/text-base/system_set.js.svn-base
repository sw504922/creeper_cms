$(function(){
	...
	$(".remove_detect").click(function(){//解除绑定弹出层
	  ...
	  $(".close_Bypass").live("click",function () //分流状态的切换
	   {
	  ...
	  $(".open_Bypass").live("click",function () 
	   {
    ...
    $(".sureEvent").live("click",function(){//分配角色提交
	   ...
	   '<span class="fileimg"></span>';
		$(".box_1").append(shtml);
	      ...
	      setTimeout(location.href="index.php?m=&c=SystemApprove&a=index&val="+adress+"&uid="+uid,2000);
   ...
   $(".halfrejectjump").live("click",function(){//确认中途驳回
	   ...
	   '<span class="fileimg"></span>';
	    ...
	    setTimeout(location.href="index.php?m=&c=SystemApprove&a=reject&uid="+uid,2000);
		});
    $(".close_reject").live("click",function(){//分配角色提交
        var uid=$('.reject_cl').attr("value");
		location.href="reject.html?uid="+uid;
		});
    $(".clearaway").live("click",function(){//清除
        var uid=$(this).attr("id");
		location.href="index.php?m=&c=SystemApprove&a=del&uid="+uid;
		});
